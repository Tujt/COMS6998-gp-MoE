...
class WandbLoggingCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        if state.is_world_process_zero:
            ...
            # 合并指标并记录
            combined_metrics = {**self.train_metrics, **self.eval_metrics}
            if combined_metrics:
                wandb.log(combined_metrics, step=state.global_step)
...
class TrainingArguments(HFTrainingArguments):
    ...
    report_to: str = field(default="wandb")  # 新增参数
    run_name: str = field(default="llama3-finetune")  # 新增参数
    wandb_project: str = field(default="llama3-experiments")  # 新增参数
    wandb_entity: Optional[str] = field(default=None)  # 新增参数
